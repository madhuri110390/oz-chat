...
class VectorFlipperProxy @Inject constructor(
    ...
    override fun init(matrix: Matrix) {
        ...
        // ✅ Add 16 KB page size check
        if (is16KBPageSize()) {
            // Skip Flipper on 16 KB devices
            return
        }
    ...
    private fun is16KBPageSize(): Boolean {
        return try {
            val pageSize = Runtime.getRuntime().exec("getconf PAGE_SIZE").inputStream.bufferedReader().readText().trim()
            pageSize == "16384"
        } catch (e: Exception) {
            false
        }
