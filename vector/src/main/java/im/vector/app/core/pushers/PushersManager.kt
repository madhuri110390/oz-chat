...
//    }
    ...
    suspend fun enqueueRegisterPusher(
        ...
        val currentSession = activeSessionHolder.getOrInitializeSession()
                ?: run {
                    // Session not ready yet — caller should retry after login
                    return null
                }
        val pusher = createHttpPusher(currentSession = currentSession, pushKey = pushKey, gateway = gateway)
    ...
    suspend fun enqueueRegisterPusherWithFcmKey(pushKey: String): UUID?  {
        return enqueueRegisterPusher(
                ...
                gateway = mdmService.getData(
                        MdmData.DefaultPushGatewayUrl,
                        stringProvider.getString(im.vector.app.config.R.string.pusher_http_url)
                )
        )
    }

    suspend fun registerPusherWithFcmKey(pushKey: String) {
        val currentSession = activeSessionHolder.getOrInitializeSession() ?: return
        val gateway = mdmService.getData(
                MdmData.DefaultPushGatewayUrl,
                stringProvider.getString(im.vector.app.config.R.string.pusher_http_url)
        )
        val pusher = createHttpPusher(currentSession = currentSession, pushKey = pushKey, gateway = gateway)
        currentSession.pushersService().addHttpPusher(pusher)
    }

    suspend fun unregisterStalePushers(currentToken: String) {
        val currentSession = activeSessionHolder.getSafeActiveSession() ?: return
        val deviceId = currentSession.sessionParams.deviceId
        val appId = stringProvider.getString(im.vector.app.config.R.string.pusher_app_id)
        
        val pushers = currentSession.pushersService().getPushers()
        for (pusher in pushers) {
            if (pusher.deviceId == deviceId && pusher.appId == appId && pusher.pushKey != currentToken) {
                try {
                    currentSession.pushersService().removeHttpPusher(pusher.pushKey, appId)
                } catch (e: Exception) {
                    // Ignore failures on unregister
                }
            }
        }
    }
    ...
    private suspend fun createHttpPusher(
            ...
            url = gateway,
            enabled = true,
            deviceId = currentSession.sessionParams.deviceId,
            append = false,
