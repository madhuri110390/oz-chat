...
abstract class VectorBaseActivity<VB : ViewBinding> : AppCompatActivity(), MavericksView {
    ...
    @Inject open lateinit var vectorPreferences: VectorPreferences
...
/*            // Adjust your layout to respect system bars (insets)
            views.root.setOnApplyWindowInsetsListener { view, insets ->
                val statusBarInsets = insets.getInsets(WindowInsets.Type.statusBars())
                view.setPadding(
                        0,
                        statusBarInsets.top, // top padding for notch/status bar
                        0,
                        0 // no bottom padding
                )
                insets
    ...
    open fun displayInFullscreen() = true
    ...
    override fun onTrimMemory(level: Int) {
        ...
        if (level == android.content.ComponentCallbacks2.TRIM_MEMORY_UI_HIDDEN || level >= android.content.ComponentCallbacks2.TRIM_MEMORY_BACKGROUND) {
            Timber.i("onTrimMemory: App is entering background (level $level). Clearing cache...")
            lifecycleScope.launch(Dispatchers.Main) {
                try {
                    com.bumptech.glide.Glide.get(this@VectorBaseActivity).clearMemory()
                    if (activeSessionHolder.hasActiveSession()) {
                        activeSessionHolder.getSafeActiveSession()?.fileService()?.clearCache()
                    }
                    withContext(Dispatchers.IO) {
                        com.bumptech.glide.Glide.get(this@VectorBaseActivity).clearDiskCache()
                    }
                } catch (e: Exception) {
                    Timber.e(e, "Failed to clear cache in background")
                }
            }
        }
    }
