...
class RoomGroupMessageCreator @Inject constructor(
    ...
    fun createRoomMessage(
        ...
        val roomIcon = lastKnownRoomEvent.roomAvatarPath?.let { bitmapLoader.getUserIcon(it) }
        notificationUtils.updateShortcut(roomId, roomName, roomIcon)
        ...
        val notification = if (useCompleteNotificationFormat) {
            ...
            notificationUtils.buildSimpleEventNotification(
                    SimpleNotifiableEvent(
                            matrixID = lastKnownRoomEvent.matrixID ?: "",
                            eventId = lastKnownRoomEvent.eventId,
                            editedEventId = lastKnownRoomEvent.editedEventId,
                            noisy = meta.shouldBing,
                            timestamp = lastMessageTimestamp,
                            description = stringProvider.getString(CommonStrings.notification_new_messages),
                            title = roomName,
                            type = lastKnownRoomEvent.type,
                            soundName = lastKnownRoomEvent.soundName,
                            canBeReplaced = false
                    ),
                    lastKnownRoomEvent.matrixID ?: ""
            )
        }

        return RoomNotification.Message(notification, meta)
    }
