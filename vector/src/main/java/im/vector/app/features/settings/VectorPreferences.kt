...
class VectorPreferences @Inject constructor(
    ...
    companion object {
        ...
        const val DID_ASK_NOTIFICATION_PERMISSION = "DID_ASK_NOTIFICATION_PERMISSION"
...
//        const val SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY = "SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY"
        ...
        const val SETTINGS_SHOW_AVATAR_DISPLAY_NAME_CHANGES_MESSAGES_KEY = "SETTINGS_SHOW_AVATAR_DISPLAY_NAME_CHANGES_MESSAGES_KEY"
        ...
        const val DEFERRED_LINK = "DEFERRED_LINK"

        const val DELETED_FOR_ME_KEY = "im.openzipper.ozchat.deleted_for_me"
    ...
    fun isSilentNotificationEnabled(): Boolean {
        return defaultPrefs.getBoolean("silent_notifications", false)
    }
    fun didAskNotificationPermission(): Boolean {
        return defaultPrefs.getBoolean(DID_ASK_NOTIFICATION_PERMISSION, false)
    }

    fun setDidAskNotificationPermission() {
        defaultPrefs.edit().putBoolean(DID_ASK_NOTIFICATION_PERMISSION, true).apply()
    }
    ...
    fun setAvatarDisplayNameChangeMessagesVisible(isVisible: Boolean) {
        defaultPrefs.edit {
            putBoolean(SETTINGS_SHOW_AVATAR_DISPLAY_NAME_CHANGES_MESSAGES_KEY, isVisible)
        }
    }
    ...
    var deferredLink: String?
        get() = defaultPrefs.getString(DEFERRED_LINK, null)
        set(value) = defaultPrefs.edit { putString(DEFERRED_LINK, value) }


    fun getRingtoneForMessage(messageId: String): Uri? {
        val prefs = context.getSharedPreferences("ringtone_prefs", Context.MODE_PRIVATE)
        val uriString = prefs.getString("ringtone_$messageId", null)
        return uriString?.let { Uri.parse(it) }
    }

    fun saveRingtoneForMessage(messageId: String, uri: Uri) {
        val prefs = context.getSharedPreferences("ringtone_prefs", Context.MODE_PRIVATE)

        prefs.edit()
                .putString("ringtone_$messageId", uri.toString())
                .apply()
    }
    fun getRoomNotificationTone(roomId: String): Uri? {
        val value = defaultPrefs.getString("room_tone_$roomId", null)
        return value?.let { Uri.parse(it) }
    }

    fun setRoomNotificationTone(roomId: String, uri: Uri?) {
        defaultPrefs.edit()
                .putString("room_tone_$roomId", uri?.toString())
                .apply()
    }
