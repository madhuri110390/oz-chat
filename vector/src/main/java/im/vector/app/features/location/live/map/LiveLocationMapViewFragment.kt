...
class LiveLocationMapViewFragment :
    ...
    private var maplibreMap: WeakReference<MapLibreMap>? = null
    ...
    private fun handleZoomToUserLocationEvent(event: LiveLocationMapViewEvents.ZoomToUserLocation) {
        maplibreMap?.get().zoomToLocation(event.userLocation)
    }
    ...
    private fun setupMap() {
        ...
        mapFragment.getMapAsync { maplibreMap ->
            ...
            lifecycleScope.launch {
                maplibreMap.setStyle(urlMapProvider.getMapUrl()) { style ->
                    mapStyle = style
                    this@LiveLocationMapViewFragment.maplibreMap = WeakReference(maplibreMap)
                    symbolManager = SymbolManager(mapFragment.view as MapView, maplibreMap, style).apply {
    ...
    private fun updateCopyrightMargin(bottomOffset: Int) {
        getOrCreateSupportMapFragment().getMapAsync { maplibreMap ->
    ...
    private fun adjustCompassButton() {
        ...
        locateButton.post {
            ...
            maplibreMap?.get()?.uiSettings?.setCompassMargins(0, marginTop, marginRight, 0)
        }
    }
    ...
    private fun updateMapZoomWhenNeeded(userLiveLocations: List<UserLiveLocationViewState>, latLngBoundsBuilder: LatLngBounds.Builder) {
        if (userLiveLocations.isNotEmpty() && isMapFirstUpdate) {
            ...
            if (userLiveLocations.size > 1) {
                maplibreMap?.get()?.zoomToBounds(latLngBoundsBuilder.build())
            } else {
                maplibreMap?.get()?.zoomToLocation(userLiveLocations.first().locationData)
            }
        }
    }
    ...
    private fun handleBottomSheetUserSelected(userId: String) = withState(viewModel) { state ->
        state.userLocations
                ...
                ?.locationData
                ?.let { locationData ->
                    maplibreMap?.get()?.zoomToLocation(locationData, preserveCurrentZoomLevel = false)
                }
    }
