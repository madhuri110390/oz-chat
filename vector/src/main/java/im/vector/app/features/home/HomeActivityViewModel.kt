...
class HomeActivityViewModel @AssistedInject constructor(
    ...
    private fun initThreadsMigration() {
        // Migration to threads is complete. We no longer need to check or force clear cache 
        // which was causing the app to restart / clear cache repeatedly.
        // Uncommented as requested by user. Added safety mechanism to prevent infinite loop.
        when {
            !vectorPreferences.areThreadMessagesEnabled() && !vectorPreferences.wasThreadFlagChangedManually() -> {
                ...
                vectorPreferences.setShouldMigrateThreads(false) // Safety to prevent loop
                // Clear Cache
    ...
    private fun checkSessionPushIsOn() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            // We no longer prompt the user to enable session push manually since we rely on native Android permission flows.
        }
    }
    ...
    override fun handle(action: HomeActivityViewActions) {
        when (action) {
            ...
            HomeActivityViewActions.ToggleFollowOptions -> {
                setState { copy(showFollowOptions = !showFollowOptions) }
            }
        }
    }
}
