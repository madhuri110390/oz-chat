...
//        val isSpace = summary?.roomType == RoomType.SPACE
//        val prefix = if (isSpace) "space" else "room"
//        val alias = summary?.canonicalAlias
//                ?: summary?.roomId
//                ?: initialState.roomId
//
//        val permalink = "$alias"
...
//    }
    private fun handleShareRoomProfile() {
        ...
        val permalink = session.permalinkService().createPermalink(alias) ?: alias
        val text = stringProvider.getString(CommonStrings.share_room_text, permalink)

        _viewEvents.post(
                RoomProfileViewEvents.ShareRoomProfile(text)
        )
    }
