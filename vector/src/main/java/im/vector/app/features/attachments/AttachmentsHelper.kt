...
class AttachmentsHelper(
    ...
    /**
     * Starts the process for handling image/video capture. Directly
     */

    fun takePhotoDirectly(cameraActivityResultLauncher: ActivityResultLauncher<Intent>) = doSafe {
        captureUri = MultiPicker.get(MultiPicker.CAMERA).startWithExpectingFile(context, cameraActivityResultLauncher)
    }

    fun recordVideoDirectly(cameraVideoActivityResultLauncher: ActivityResultLauncher<Intent>) = doSafe {
        captureUri = MultiPicker.get(MultiPicker.CAMERA_VIDEO).startWithExpectingFile(context, cameraVideoActivityResultLauncher)
    }

    /**
     * Starts the process for handling image/video capture. Can open a dialog not for oz-chat
     */
    fun openCamera(
            activity: Activity,
