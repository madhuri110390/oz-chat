...
class MessageActionsViewModel @AssistedInject constructor(
    ...
    private fun getRedactionReason(timelineEvent: TimelineEvent): String {
        return (timelineEvent
                ...
                ?.takeIf { it.isNotBlank() }
                .let { reason ->
                    if (reason == null && timelineEvent.root.isRedactedBySameUser()) {
    ...
    private fun ArrayList<EventSharedAction>.addActionsForSentNotSyncedState(timelineEvent: TimelineEvent) {
        ...
        if (canDeleteForMe(timelineEvent)) {
            add(EventSharedAction.DeleteForMe(timelineEvent.eventId))
        }
    ...
    private suspend fun ArrayList<EventSharedAction>.addActionsForSyncedState(
        ...
        if (!timelineEvent.root.isRedacted()) {
            ...
            if (canDeleteForMe(timelineEvent)) {
                add(EventSharedAction.DeleteForMe(eventId))
            }
...
//        if (!vectorPreferences.areThreadMessagesEnabled()) return false
    ...
    private fun canDeleteForMe(event: TimelineEvent): Boolean {
        // You can fine-tune this logic! Here’s a simple example:

        // 1. Hide if the message is already redacted (deleted for everyone)
        if (event.root.isRedacted()) return false

        // 2. Optionally, hide if this eventId is already in the deleted-for-me list
        // (We’ll implement this after step 4, when the filter list is available)
        // For now, always return true to show for all events
