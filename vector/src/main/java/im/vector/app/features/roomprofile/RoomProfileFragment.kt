    ...
    override fun invalidate() = withState(roomProfileViewModel) { state ->
        ...
        state.roomSummary()?.let {
            if (it.membership.isLeft()) {
                ...
                val bestName = if (it.isDirect) {
                    it.directUserId?.removePrefix("@")?.substringBefore(":") ?: matrixItem.getBestName()
                } else {
                    matrixItem.getBestName()
                }
                headerViews.roomProfileNameView.text = bestName
                views.matrixProfileToolbarTitleView.text = bestName
                val cleanAlias = it.canonicalAlias?.let { alias ->
                    val name = if (alias.contains(":")) alias.substring(0, alias.indexOf(":")) else alias
                    name.removePrefix("/profile/")
                }
                headerViews.roomProfileAliasView.setTextOrHide(cleanAlias)
