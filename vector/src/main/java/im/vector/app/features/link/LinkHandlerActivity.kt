...
class LinkHandlerActivity : VectorBaseActivity<ActivityProgressBinding>() {
    ...
    @Inject override lateinit var vectorPreferences: im.vector.app.features.settings.VectorPreferences
    ...
    private fun handleIntent() {
        Firebase.dynamicLinks
                ...
                .addOnSuccessListener(this) { pendingDynamicLinkData ->
                    ...
                    if (deepLink != null) {
                        Timber.d("Dynamic link resolved to: $deepLink")
                        processUri(deepLink)
                    } else {
                        val uri = intent.data
                        if (uri != null) {
                ...
                .addOnFailureListener(this) { e ->
                    Timber.e(e, "getDynamicLink:onFailure")
                    val uri = intent.data
                    if (uri != null) {
                        processUri(uri)
                    } else {
    ...
    private fun processUri(uri: Uri) {
        when {
            ...
            uri.toString().startsWith(PermalinkService.MATRIX_TO_URL_BASE) -> handleSupportedHostUrl(uri)
            uri.toString().startsWith(PermalinkHandler.MATRIX_TO_CUSTOM_SCHEME_URL_BASE) -> handleSupportedHostUrl(uri)
            resources.getStringArray(im.vector.app.config.R.array.permalink_supported_hosts).contains(uri.host) -> handleSupportedHostUrl(uri)
            else -> {
    ...
    private fun handleSupportedHostUrl(uri: Uri) {
        ...
        if (!activeSessionHolder.hasActiveSession()) {
            ...
            vectorPreferences.deferredLink = link
            startLoginActivity()
            return
        }
