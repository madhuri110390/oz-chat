...
//    }
    ...
    private fun updateEnabledForAccount(preference: Preference?) {
        ...
        val isChecked = switchPref.isChecked

        lifecycleScope.launch {
            val rules = pushRuleService.getPushRules().getAllRules()
            val mRuleMaster = rules.find { it.isDisableAll() }

            if (mRuleMaster != null) {
                try {
                    // We must ENABLE this rule to DISABLE notifications
                    // So if the switch is ON (notifications enabled), we want to DISABLE the rule
                    // If the switch is OFF (notifications disabled), we want to ENABLE the rule
                    ...
                    switchPref.isChecked = !isChecked
