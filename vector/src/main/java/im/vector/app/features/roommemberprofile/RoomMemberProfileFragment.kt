...
class RoomMemberProfileFragment :
    ...
    override fun invalidate() = withState(viewModel) { state ->
        when (val asyncUserMatrixItem = state.userMatrixItem) {
            Uninitialized,
            is Loading -> {
                val loadingName = state.userId.removePrefix("@").substringBefore(":")
                views.matrixProfileToolbarTitleView.text = loadingName
            ...
            is Fail -> {
                avatarRenderer.render(MatrixItem.UserItem(state.userId, null, null), views.matrixProfileToolbarAvatarImageView)
                views.matrixProfileToolbarTitleView.text = state.userId.removePrefix("@").substringBefore(":")
            ...
            is Success -> {
                ...
                val username = state.userId.removePrefix("@").substringBefore(":")
                headerViews.memberProfileNameView.text = username
                headerViews.memberProfileNameView.setTextColor(matrixItemColorProvider.getColor(userMatrixItem))
                views.matrixProfileToolbarTitleView.text = username
        ...
        // Hide power level (Admin/Moderator label) per product requirement
        // headerViews.memberProfilePowerLevelView.setTextOrHide(state.userPowerLevelString())
        headerViews.memberProfilePowerLevelView.setTextOrHide(null)
    ...
    override fun onIgnoreClicked() = withState(viewModel) { state ->
        ...
        if (isIgnored) {
            ...
            positiveButtonRes = CommonStrings.room_participants_action_ignore_title
        }
    ...
    private fun handleShareRoomMemberProfile(permalink: String) {
        ...
        MaterialAlertDialogBuilder(requireContext())
                ...
                .setPositiveButton(CommonStrings.share_by_text) { _, _ ->
                    val text = getString(CommonStrings.share_user_profile_text, permalink)
    ...
    private fun onAvatarClicked(view: View, userMatrixItem: MatrixItem) {
        val username = userMatrixItem.id.removePrefix("@").substringBefore(":")
        navigator.openBigImageViewer(requireActivity(), view, userMatrixItem.avatarUrl, username)
    }
