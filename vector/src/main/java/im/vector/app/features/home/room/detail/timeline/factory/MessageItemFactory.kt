...
class MessageItemFactory @Inject constructor(
    ...
    private fun resolveDisplayName(userId: String): String {
        if (!userId.startsWith("@")) return "Unknown"
        ...
        return userId.substringAfter("@").substringBefore(":")
    }

    private fun buildResolvedMatrixItem(
            userId: String,
            displayName: String? = null,
            avatarUrl: String? = null,
    ): MatrixItem.UserItem {
        val resolvedName = displayName?.takeIf { it.isNotBlank() } ?: resolveDisplayName(userId)
        return MatrixItem.UserItem(id = userId, displayName = resolvedName, avatarUrl = avatarUrl)
    }

    fun getInitials(displayName: String): String {
        val cleaned = if (displayName.startsWith("@")) displayName.substringAfter("@").substringBefore(":") else displayName
    ...
    private fun buildVoiceMessageItem(
        ...
        val fileUrl = getAudioFileUrl(messageContent, informationData)
        val playbackControlButtonClickListener = createOnPlaybackButtonClickListener(messageContent, informationData, params)
        val duration = messageContent.audioWaveformInfo?.duration
                ?: messageContent.audioInfo?.duration
                ?: 0
...
//        }
        val otherUserName: String = if (informationData.sentByMe) {
            session.roomService().getRoomMember(messageContent.toUserId, roomId)
                    ?.displayName?.takeIf { it.isNotBlank() }
                    ?: resolveDisplayName(messageContent.toUserId)
        } else {
            informationData.memberName?.toString()?.takeIf { it.isNotBlank() }
                    ?: resolveDisplayName(informationData.senderId)
        }
        val otherMatrixItem = buildResolvedMatrixItem(
                userId = otherUserId,
                displayName = otherUserName,
                avatarUrl = session.roomService().getRoomMember(otherUserId, roomId)?.avatarUrl,
        )
        ...
        return VerificationRequestItem_()
                .attributes(
                        VerificationRequestItem.Attributes(
                                ...
                                otherMatrixItem = otherMatrixItem,
...
//        formattedBody.append("* ${informationData.memberName} ")
        val resolvedSenderName = informationData.memberName
                ?.toString()?.takeIf { it.isNotBlank() }
                ?: resolveDisplayName(informationData.senderId)
        formattedBody.append("* $resolvedSenderName ")
