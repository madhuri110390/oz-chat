...
class OnboardingViewModel @AssistedInject constructor(
    ...
    private fun handleResetPassword(action: OnboardingAction.ResetPassword) {
        _viewEvents.post(OnboardingViewEvents.OpenExternalUrl("https://openzippers.com/login"))
    }
    ...
    private fun handleLogin(action: AuthenticateAction.Login) {
        val safeLoginWizard = loginWizard
        setState { copy(isLoading = true) }
        // Strip full Matrix ID to plain username (e.g. @john:server.com → john)
        // Email (john@gmail.com) and plain usernames (john) pass through unchanged
        val loginInput = action.username.trim()
        val resolvedLogin = if (loginInput.startsWith("@") && loginInput.contains(":")) {
            loginInput.substring(1, loginInput.indexOf(":"))
        } else {
            loginInput
        }
        currentJob = viewModelScope.launch {
