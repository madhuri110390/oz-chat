...
class NoticeEventFormatter @Inject constructor(
    ...
    fun format(timelineEvent: TimelineEvent, isDm: Boolean): CharSequence? {
        ...
        val senderName = timelineEvent.senderInfo.userId.removePrefix("@").substringBefore(":")
    ...
    private fun formatRoomPowerLevels(event: Event, disambiguatedDisplayName: String): CharSequence? {
        ...
        userIds.forEach { userId ->
            ...
            if (from != to) {
                ...
                val displayName = userId.removePrefix("@").substringBefore(":")
            ...
            val displayNameText = when {
                prevEventContent?.displayName.isNullOrEmpty() ->
                    if (event.isSentByCurrentUser()) {
                        ...
                        sp.getString(CommonStrings.notice_display_name_set, event.senderId?.removePrefix("@")?.substringBefore(":"), eventContent?.displayName)
                    }
                eventContent?.displayName.isNullOrEmpty() ->
                    if (event.isSentByCurrentUser()) {
                        ...
                        sp.getString(CommonStrings.notice_display_name_removed, event.senderId?.removePrefix("@")?.substringBefore(":"), prevEventContent?.displayName)
                    }
                else ->
                    if (event.isSentByCurrentUser()) {
    ...
    private fun buildMembershipNotice(
        ...
        val senderDisplayName = event.senderId?.removePrefix("@")?.substringBefore(":") ?: ""
        val targetDisplayName = event.stateKey?.removePrefix("@")?.substringBefore(":") ?: ""
    ...
    fun formatRedactedEvent(event: Event): String {
        return (event
                ...
                .let { reason ->
                    if (reason == null && event.isRedactedBySameUser()) {
