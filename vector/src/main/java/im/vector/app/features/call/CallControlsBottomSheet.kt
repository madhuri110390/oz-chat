...
class CallControlsBottomSheet : VectorBaseBottomSheetDialogFragment<BottomSheetCallControlsBinding>() {
   ...
   // val isInitialVideoCall: Boolean = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        views.callControlsInviteFriends.views.bottomSheetActionClickableZone.debouncedClicks {
            callViewModel.handle(VectorCallViewActions.OpenInviteFriends)
            dismiss()
        }
        ...
        views.callControlsShareScreen.views.bottomSheetActionClickableZone.debouncedClicks {
            Timber.d(" User tapped 'Share Screen' → Dispatching ToggleScreenSharing")
            callViewModel.handle(VectorCallViewActions.ToggleScreenSharing)
            dismiss()
        }
        views.callSwitchVideoToVoice.views.bottomSheetActionClickableZone.debouncedClicks {
            callViewModel.handle(VectorCallViewActions.SwitchToVoice)
            dismiss()
        }

        views.callSwitchVoiceToVideo.views.bottomSheetActionClickableZone.debouncedClicks {
            callViewModel.handle(VectorCallViewActions.SwitchToVideo)
            dismiss()
        }
    ...
    private fun renderState(state: VectorCallViewState) {
        ...
        if (state.isVideoCall) {
            ...
            views.callSwitchVideoToVoice.isVisible = true
            views.callSwitchVoiceToVideo.isVisible = false
            if (state.isHD) {
            ...
            views.callSwitchVideoToVoice.isVisible = false
            views.callSwitchVoiceToVideo.isVisible = true
        }
