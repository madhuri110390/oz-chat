...
class RoomMemberProfileViewModel @AssistedInject constructor(
    ...
    private fun handleOpenOrCreateDm(action: RoomMemberProfileAction.OpenOrCreateDm) {
        viewModelScope.launch {
            ...
            Timber.d("ensureDMExists result for user ${action.userId}: $roomId")
            if (!roomId.startsWith("!")) {
                Timber.e("WTF: ensureDMExists returned something that is not a roomId: $roomId")
            }
            _viewEvents.post(RoomMemberProfileViewEvents.Loading(null))
    ...
    private fun handleShareRoomMemberProfile() {
        val permalink = session.permalinkService().createPermalink(initialState.userId) ?: initialState.userId
