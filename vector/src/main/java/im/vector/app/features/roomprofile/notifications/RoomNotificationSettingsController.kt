...
class RoomNotificationSettingsController @Inject constructor() : TypedEpoxyController<RoomNotificationSettingsViewState>() {
    ...
    interface Callback {
        ...
        fun onRingtoneClicked()
    }

    var callback: Callback? = null
    private var currentToneName: String? = null
    fun setToneName(name: String) {
        currentToneName = name
    ...
    override fun buildModels(data: RoomNotificationSettingsViewState?) {
        ...
        buildProfileAction(
                id = "ringtone",
                title = "Call Ring Tone",
                subtitle = currentToneName ?: "Default",
                editable = true,
                action = { callback?.onRingtoneClicked() }
        )
