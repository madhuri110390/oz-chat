...
class PermalinkHandler @Inject constructor(
    ...
    suspend fun launch(
        ...
        val isCustomDomain = deepLink?.host == "openzipper.com" || deepLink?.host == "openzippers.com" || deepLink?.host == "oz.openzippers.com" || deepLink?.host == "oz-chat.app" || deepLink?.host == "ozchat.app" || deepLink?.host == "chat.openzippers.com"
        
        return when {
            deepLink == null -> false
            deepLink.isIgnored() -> true
            isCustomDomain -> {
                handleCustomDomain(deepLink, fragmentActivity, navigationInterceptor, buildTask)
            }
    ...
    private suspend fun handleCustomDomain(
            deepLink: Uri,
            fragmentActivity: FragmentActivity,
            navigationInterceptor: NavigationInterceptor?,
            buildTask: Boolean
    ): Boolean {
        var path = deepLink.path?.removePrefix("/") ?: ""
        if (path.startsWith("ozchat/")) {
            path = path.removePrefix("ozchat/")
        } else if (path == "ozchat") {
            path = ""
        }
        val fragment = deepLink.fragment
        val session = activeSessionHolder.getSafeActiveSession() ?: return false
        val serverName = session.myUserId.substringAfter(":")

        val (identifier, type) = when {
            path.startsWith("profile/") -> {
                val id = path.removePrefix("profile/")
                (if (id.isEmpty()) fragment ?: "" else id) to "user"
            }
            path.startsWith("room/") -> {
                val id = path.removePrefix("room/")
                (if (id.isEmpty()) fragment ?: "" else id) to "room"
            }
            path.startsWith("space/") -> {
                val id = path.removePrefix("space/")
                (if (id.isEmpty()) fragment ?: "" else id) to "space"
            }
            path.startsWith("invite") -> {
                val ref = deepLink.getQueryParameter("ref")
                (if (ref.isNullOrEmpty()) fragment ?: "" else ref) to "unknown"
            }
            else -> (if (path.isNotEmpty()) path else fragment ?: "") to "unknown"
        }

        // If the path is empty and no identifier was found, this is the basic promotional link (e.g., https://openzipper.com/ozchat)
        if (path.isEmpty() && identifier.isEmpty()) {
            // No specific action needed; the app is already opened.
            return true
        }
        if (identifier.isEmpty()) return false

        // Try to resolve as room/space first if not explicitly a user link
        if (type != "user") {
            val roomAliases = mutableListOf<String>()
            
            if (identifier.startsWith("#") || identifier.startsWith("!")) {
                roomAliases.add(identifier)
            } else {
                if (identifier.contains(":")) {
                    roomAliases.add("#$identifier")
                } else {
                    roomAliases.add("#$identifier:$serverName")
                    roomAliases.add("#$identifier:oz.openzippers.com")
                }
            }
            
            // Fallback for search compatibility if it was extracted weirdly
            val cleanIdentifier = identifier.removePrefix("#").removePrefix("!")
            if (cleanIdentifier != identifier) {
                if (cleanIdentifier.contains(":")) {
                    roomAliases.add("#$cleanIdentifier")
                } else {
                    roomAliases.add("#$cleanIdentifier:$serverName")
                }
            }

            var resolvedRoomId: String? = null
            for (alias in roomAliases.distinct()) {
                resolvedRoomId = session.roomService().getRoomIdByAlias(alias, true).getOrNull()?.roomId
                if (resolvedRoomId == null && alias.startsWith("!")) {
                    resolvedRoomId = alias
                }
                if (resolvedRoomId != null) break
            }

            if (resolvedRoomId != null) {
                val permalinkData = PermalinkData.RoomLink(
                        roomIdOrAlias = resolvedRoomId,
                        isRoomAlias = false,
                        eventId = null,
                        viaParameters = emptyList()
                )
                openRoom(navigationInterceptor, fragmentActivity, resolvedRoomId, permalinkData, deepLink, buildTask)
                return true
            }
        }

    // Fallback to user profile if not resolved as a room or explicitly a user link
    if (type != "room" && type != "space") {
        val userId = when {
            identifier.startsWith("@") -> if (identifier.contains(":")) identifier else "$identifier:$serverName"
            identifier.contains(":") -> "@$identifier"
            else -> "@$identifier:$serverName"
        }
        // Fallback userId for backward compatibility
        val fallbackUserId = when {
            identifier.startsWith("@") -> if (identifier.contains(":")) identifier else "$identifier:oz.openzippers.com"
            identifier.contains(":") -> "@$identifier"
            else -> "@$identifier:oz.openzippers.com"
        }

        val userPermalinkData = if (identifier.startsWith("@") || identifier.contains(":")) {
            PermalinkData.UserLink(userId = userId)
        } else {
            // Try to see if user exists locally first
            if (activeSessionHolder.getSafeActiveSession()?.userService()?.getUser(userId) != null) {
                PermalinkData.UserLink(userId = userId)
            } else {
                PermalinkData.UserLink(userId = fallbackUserId)
            }
        }
        return handleUserLink(userPermalinkData, deepLink, fragmentActivity, navigationInterceptor, buildTask)
    }
    ...
    private suspend fun handleUserLink(
        ...
        val isInviteLink = rawLink.toString().contains("openzipper.com") || 
                           rawLink.toString().contains("oz-chat.app") ||
                           rawLink.toString().contains("ozchat.app")

        if (isInviteLink) {
            val roomId = directRoomHelper.ensureDMExists(permalinkData.userId)
            val myUsername = activeSessionHolder.getSafeActiveSession()?.myUserId?.removePrefix("@")?.substringBefore(":") ?: ""
            val promotionalMessage = context.getString(im.vector.lib.strings.CommonStrings.invite_friends_text, myUsername)
                    .split("\n\n")
                    .firstOrNull()
                    ?.trim()
                    ?: ""
            ...
            if (context is Activity) {
                navigator.openRoomForSharingAndFinish(context, roomId, SharedData.Text(promotionalMessage))
                return true
            }
        }
