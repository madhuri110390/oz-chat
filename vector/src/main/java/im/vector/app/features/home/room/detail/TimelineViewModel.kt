...
class TimelineViewModel @AssistedInject constructor(
    ...
    private fun initSafe(room: Room, timeline: Timeline) {
        ...
        observeSummaryState()
        getUnreadState()
        observeDeletedForMeAccountData()
        observeSyncState()
        observeDataStore()
    ...
    override fun handle(action: RoomDetailAction) {
        when (action) {
            ...
            //Experimental DeleteForMe
            is RoomDetailAction.DeleteForMe -> handleDeleteForMe(action.eventId)
        ...
        when {
            ...
            initialState.isThreadTimeline() -> {
                ...
                when (itemId) {
                    ...
                    R.id.roomToolbarVoiceCall -> state.isCallOptionAvailable() || state.hasActiveElementCallWidget()
                    R.id.roomToolbarVideoCall -> state.isCallOptionAvailable() && (state.jitsiState.confId == null || state.jitsiState.hasJoined)
    ...
    private fun handleDeleteForMe(eventId: String) {
        viewModelScope.launch {
            try {
                val accountDataKey = VectorPreferences.DELETED_FOR_ME_KEY
                val accountDataService = session.accountDataService()

                val currentList = (accountDataService
                        .getUserAccountDataEvent(accountDataKey)
                        ?.content
                        ?.get("deleted_event_ids") as? List<*>)?.mapNotNull { it as? String } ?: emptyList()

                if (!currentList.contains(eventId)) {
                    val updatedList = currentList + eventId
                    accountDataService.updateUserAccountData(accountDataKey, mapOf("deleted_event_ids" to updatedList))

                    _viewEvents.post(RoomDetailViewEvents.ShowMessage(stringProvider.getString(CommonStrings.message_deleted_for_you)))
                }
            } catch (e: Exception) {
                Timber.e(e, "Failed to update account data for delete for me")
                _viewEvents.post(RoomDetailViewEvents.ShowMessage("Failed to delete message for you"))
            }
        }
    }
    ...
    private fun observeDeletedForMeAccountData() {
        session.accountDataService()
                .getLiveUserAccountDataEvent(VectorPreferences.DELETED_FOR_ME_KEY)
                .asFlow()
                .mapNotNull { it.getOrNull() }
                .map { event ->
                    (event.content["deleted_event_ids"] as? List<*>)?.mapNotNull { it as? String }?.toSet().orEmpty()
                }
                .onEach { deletedIds ->
                    setState {
                        copy(deletedForMeEventIds = deletedIds)
                    }
                }
                .launchIn(viewModelScope)
    }
