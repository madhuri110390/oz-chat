...
class ImageContentRenderer @Inject constructor(
    ...
    private fun createGlideRequest(data: Data, mode: Mode, imageView: ImageView, size: Size): RequestBuilder<Drawable> {
        return createGlideRequest(data, mode, Glide.with(imageView), size)
    }

    fun createGlideRequest(data: Data, mode: Mode, requestManager: RequestManager, size: Size = processSize(data, mode)): RequestBuilder<Drawable> {
            ...
            requestManager
                    ...
                    .apply {
                        if (mode == Mode.THUMBNAIL) {
                            error(
                                    requestManager.asDrawable().load(resolveUrl(data))
                            )
                        }
                    }
        }
