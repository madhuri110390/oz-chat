...
import im.vector.app.features.home.room.detail.timeline.helper.GroupedCallEventsBlock
...
class TimelineEventController @Inject constructor(
    ...
    data class PartialState(
            ...
            val deletedForMeEventIds: Set<String> = emptySet()
    ) {
        ...
        constructor(state: RoomDetailViewState) : this(
                ...
                deletedForMeEventIds = state.deletedForMeEventIds
        )
...
//        fun onAudioMessageClicked(messageAudioContent: MessageAudioContent)
    ...
    private val groupedCallEventBlocks = mutableMapOf<String, GroupedCallEventsBlock>()
            ...
            synchronized(modelCache) {
                ...
                val prevDisplayableEventIndex = prevList.indexOfLast {
                    timelineEventVisibilityHelper.shouldShowEvent(
                            ...
                            deletedForMeEventIds = partialState.deletedForMeEventIds
                    )
                }
...
// Timeline.LISTENER ***************************************************************************
    ...
    private fun buildCacheItemsIfNeeded() = synchronized(modelCache) {
        ...
        // ✅ SET GROUPED CALL EVENTS ONLY ONCE
        timelineItemFactory.setGroupedCallEventBlocks(groupedCallEventBlocks)
    ...
    private fun preprocessReverseEvents() {
        ...
        groupedCallEventBlocks.clear()

        //  Cache for tracking latest group per sender+status+kind
        val groupingCache = mutableMapOf<String, GroupedCallEventsBlock>()
        ...
        while (itr.hasPrevious()) {
            ...
            if (EventType.isCallEvent(event.root.getClearType())) {
                val timelineGroup = timelineEventsGroups.getOrNull(event) ?: continue
                val signalingGroup = CallSignalingEventsGroup(timelineGroup)

                val callStatus = signalingGroup.callStatus
                val callKind = signalingGroup.callKind
                val invite = signalingGroup.getInvite() ?: continue
                val senderId = invite.root.senderId ?: continue
                val timestamp = event.root.originServerTs ?: continue
                if (callStatus in listOf(
                                CallTileTimelineItem.CallStatus.MISSED,
                                CallTileTimelineItem.CallStatus.REJECTED
                        )) {

                    val groupKey = "$senderId-$callStatus-$callKind"
                    val lastGroup = groupingCache[groupKey]

                    val isWithinTimeWindow = lastGroup != null &&
                            kotlin.math.abs(lastGroup.lastTimestamp - timestamp) <= 6 * 60 * 1000 // 6 mins

                    if (isWithinTimeWindow) {
                        lastGroup?.events?.add(event)
                        lastGroup?.lastTimestamp = timestamp
                        // ✅ Map all event IDs in group to the group block
                        lastGroup?.events?.forEach {
                            groupedCallEventBlocks[it.eventId] = lastGroup
                        }
                    } else {
                        val newGroup = GroupedCallEventsBlock(
                        ...
                        //  Map event IDs to group block
                        newGroup.events.forEach {
                            groupedCallEventBlocks[it.eventId] = newGroup
                        }
                        groupingCache[groupKey] = newGroup
                    }
                }
//                else {
//                    Timber.d("⛔ Skipping callStatus=$callStatus for event=${event.root.eventId}")
//                }
            }
            ...
            if (lastShownEventId == null) continue

            readReceiptsCache.addReceiptsOnEvent(currentReadReceipts, lastShownEventId)
        }

       // Timber.d("GroupingDebug: Total grouped call blocks: ${groupedCallEventBlocks.size}")
        groupedCallEventBlocks.forEach { (key, block) ->
            Timber.d("🧩 Group: $key → ${block.events.size} events, sender: ${block.senderId}, kind: ${block.callKind}, status: ${block.callStatus}")
        }
    }
    ...
    private data class CacheItemData(
        ...
        fun isCacheable(partialState: PartialState): Boolean {
            return isCacheable &&
                    partialState.highlightedEventId != eventId &&
                    eventId !in partialState.deletedForMeEventIds
        }
    }
}
