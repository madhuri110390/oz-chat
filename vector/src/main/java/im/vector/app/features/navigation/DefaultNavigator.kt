...
class DefaultNavigator @Inject constructor(
    ...
    override fun openRoom(
        ...
        if (!MatrixPatterns.isRoomId(roomId)) {
            Timber.e("Trying to open an invalid room ID: $roomId")
            return
        }

        if (sessionHolder.getSafeActiveSession()?.getRoom(roomId) == null) {
            // Room may not yet be in local cache (e.g., right after createRoom() before sync arrives).
            // Log a warning but still navigate — RoomDetailActivity handles the loading state.
            Timber.w("openRoom: room $roomId not yet in local cache, navigating anyway")
        }
    ...
    override fun switchToSpace(
        ...
        if (sessionHolder.getSafeActiveSession()?.getRoomSummary(spaceId) == null) {
            Timber.e("Trying to open an unknown space $spaceId")
            if (vectorPreferences.failFast()) {
                fatalError("Trying to open an unknown space $spaceId", true)
            }
            return
        }
...
//        val session = sessionHolder.getSafeActiveSession() ?: return
    ...
    override fun openCreateDirectRoom(context: Context, initialShowFollowers: Boolean, initialShowFollowing: Boolean) {
        when (val currentSpace = spaceStateHandler.getCurrentSpace()) {
            null -> CreateDirectRoomActivity.getIntent(context, initialShowFollowers, initialShowFollowing)
