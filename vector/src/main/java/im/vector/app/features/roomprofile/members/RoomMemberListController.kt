...
class RoomMemberListController @Inject constructor(
    ...
    override fun buildModels(data: RoomMemberListViewState?) {
        ...
        for ((powerLevelCategory, roomMemberList) in roomMembersByPowerLevel) {
            ...
            if (powerLevelCategory != RoomMemberListCategories.USER && powerLevelCategory != RoomMemberListCategories.INVITE) {
                // Skip headers for Admin, Moderator, Custom to avoid "monitoring" impression
            } else {
                buildProfileSection(
