...
class TimelineEventsGroups {
    ...
    private fun TimelineEvent.getGroupIdOrNull(): String? {
       ...
       // Timber.d("GroupingDebug: eventId=${root.eventId}, type=$type, groupId=$groupId")
        return groupId
    }

    fun events(): List<TimelineEventsGroup> {
...
class CallSignalingEventsGroup(private val group: TimelineEventsGroup) {
    ...
    val callStatus: CallTileTimelineItem.CallStatus
        get() = when {
            callWasAnswered() && getHangup() != null -> CallTileTimelineItem.CallStatus.ENDED
            getReject() != null -> CallTileTimelineItem.CallStatus.REJECTED
            getHangup() != null -> CallTileTimelineItem.CallStatus.MISSED
            isRinging() -> CallTileTimelineItem.CallStatus.INVITED
            else -> CallTileTimelineItem.CallStatus.INVITED
        }
    ...
    fun formattedDuration(): String {
        ...
        return if (start == null || end == null) {
            ...
            TextUtils.formatCallDuration(duration)
        }
    }

    fun isCallPlacedBy(userId: String): Boolean {
        return getInvite()?.root?.senderId == userId
    }
