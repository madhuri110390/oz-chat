...
class TimelineItemFactory @Inject constructor(
     ...
     */
    private var groupedCallEventBlocks: Map<String, GroupedCallEventsBlock> = emptyMap()
    fun setGroupedCallEventBlocks(blocks: Map<String, GroupedCallEventsBlock>) {
        this.groupedCallEventBlocks = blocks
    }
     ...
     fun create(params: TimelineItemFactoryParams): VectorEpoxyModel<*> {
        ...
        val computedModel = try {
            if (!timelineEventVisibilityHelper.shouldShowEvent(
                            ...
                            deletedForMeEventIds = params.deletedForMeEventIds
                    )) {
                return buildEmptyItem(
                        event,
                        params.deletedForMeEventIds,
                        params.prevEvent,
            ...
            if (event.root.isStateEvent()) {
                ...
                when (event.root.getClearType()) {
                    ...
                    // Calls
                    EventType.CALL_INVITE->callItemFactory.create(params)
                    EventType.CALL_HANGUP->{
                        val groupBlock = groupedCallEventBlocks[params.event.eventId]
                        when {
                            groupBlock == null -> callItemFactory.create(params)
                            params.event.eventId == groupBlock.representativeEventId ->
                                callItemFactory.create(params, groupedCallCount = groupBlock.events.size)
                            else -> null
                        }
                    }
                    EventType.CALL_REJECT->{
                        val groupBlock = groupedCallEventBlocks[params.event.eventId]
                     //   Timber.d("GroupingDebug: eventId=${params.event.eventId}, groupBlock=${groupBlock != null}, isRepresentative=${groupBlock?.representativeEventId == params.event.eventId}, groupSize=${groupBlock?.events?.size}")
                        when {
                            groupBlock == null -> callItemFactory.create(params)
                            params.event.eventId == groupBlock.representativeEventId ->
                                callItemFactory.create(params, groupedCallCount = groupBlock.events.size)
    ...
    private fun buildEmptyItem(
            ...
            deletedForMeEventIds: Set<String>,
        ...
        val isNotBlank = prevEvent == null || timelineEventVisibilityHelper.shouldShowEvent(
                ...
                deletedForMeEventIds = deletedForMeEventIds
        )
