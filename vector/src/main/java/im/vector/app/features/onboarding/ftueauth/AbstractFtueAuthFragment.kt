...
abstract class AbstractFtueAuthFragment<VB : ViewBinding> : VectorBaseFragment<VB>(), OnBackPressed {
    ...
    var hasOpenedForgotPassword = false
    ...
    open fun updateWithState(state: OnboardingViewState) {
        if (isResetPasswordStarted && !hasOpenedForgotPassword) {
            hasOpenedForgotPassword = true

            openUrlInChromeCustomTab(
                    requireContext(),
                    null,
                    "https://openzippers.com/forgot-password"
            )
        }
    }
