...
//                val permalink = "$MATRIX_TO_URL_BASE + spaceId"
...
override fun handle(action: ShareSpaceAction) {
    when (action) {
        ShareSpaceAction.InviteByLink -> {
            ...
            val alias = roomSummary?.canonicalAlias
                    ?: initialState.spaceId   // fallback to roomId

            val permalink = session.permalinkService().createPermalink(alias) ?: alias

            _viewEvents.post(
                    ShareSpaceViewEvents.ShowInviteByLink(
                            permalink,
                            roomSummary?.name ?: ""
                    )
            )
        ...
        ShareSpaceAction.InviteByMxId -> {
            _viewEvents.post(
                    ShareSpaceViewEvents.NavigateToInviteUser(
                            initialState.spaceId
                    )
            )
        }
