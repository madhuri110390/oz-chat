...
class CallControlsView @JvmOverloads constructor(
    ...
    private fun toggleCamera() {
        interactionListener?.didTapToggleCamera()
    }
    ...
    fun updateForState(state: VectorCallViewState) {
        ...
        views.videoToggleIcon.isEnabled = !state.isLocalScreenSharing
        views.videoToggleIcon.alpha = if (state.isLocalScreenSharing) 0.5f else 1f
        ...
        when (callState) {
            ...
            is CallState.Answering -> {
                ...
                views.audioSettingsIcon.isVisible = !state.isVideoCall
                views.moreIcon.isVisible = callState is CallState.Connected && (callState as? CallState.Connected)?.iceConnectionState == MxPeerConnectionState.CONNECTED
            }
            is CallState.Ended -> {
                views.ringingControls.isVisible = false
                views.connectedControls.isVisible = false
