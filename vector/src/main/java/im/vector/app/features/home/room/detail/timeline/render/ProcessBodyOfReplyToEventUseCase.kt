        ...
        val withTranslatedContent = if (repliedToEvent != null && breakingLineIndex != -1 && endOfBlockQuoteIndex != -1) {
            ...
            when {
                ...
                repliedToEvent.isLiveLocation() -> {
                    ...
                    val originalQuotedText = matrixFormattedBody.substring(afterBreakingLineIndex, endOfBlockQuoteIndex)
                    val truncated = if (originalQuotedText.length > 50) {
                        originalQuotedText.take(50).trimEnd() + "…"
                    } else originalQuotedText

                    matrixFormattedBody.replaceRange(afterBreakingLineIndex, endOfBlockQuoteIndex, truncated)
                }
            }
        } else {
            matrixFormattedBody
