...
class CreateDirectRoomActivity : SimpleFragmentActivity() {
        ...
        if (isFirstCreation()) {
            addFragment(
                    ...
                    UserListFragmentArgs(
                            ...
                            initialShowFollowers = intent.getBooleanExtra(EXTRA_INITIAL_SHOW_FOLLOWERS, false),
                            initialShowFollowing = intent.getBooleanExtra(EXTRA_INITIAL_SHOW_FOLLOWING, false)
                    )
            )
        }
    ...
    companion object {
        private const val EXTRA_INITIAL_SHOW_FOLLOWERS = "EXTRA_INITIAL_SHOW_FOLLOWERS"
        private const val EXTRA_INITIAL_SHOW_FOLLOWING = "EXTRA_INITIAL_SHOW_FOLLOWING"

        fun getIntent(
                context: Context,
                initialShowFollowers: Boolean = false,
                initialShowFollowing: Boolean = false
        ): Intent {
            return Intent(context, CreateDirectRoomActivity::class.java).apply {
                putExtra(EXTRA_INITIAL_SHOW_FOLLOWERS, initialShowFollowers)
                putExtra(EXTRA_INITIAL_SHOW_FOLLOWING, initialShowFollowing)
            }
        }
    }
}
