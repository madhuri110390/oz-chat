...
class MainActivity : VectorBaseActivity<ActivityMainBinding>(), UnlockedActivity {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val deferredLink = vectorPreferences.deferredLink
        if (deferredLink != null) {
            vectorPreferences.deferredLink = null
            val intent = Intent(this, im.vector.app.features.link.LinkHandlerActivity::class.java)
            intent.data = android.net.Uri.parse(deferredLink)
    ...
    private fun handleStartForegroundService() {
        if (startAppViewModel.shouldStartApp()) {
            ...
            try {
                ...
                Timber.e(ex, "Could not start StartAppAndroidService in background")
            }
        }
    }
