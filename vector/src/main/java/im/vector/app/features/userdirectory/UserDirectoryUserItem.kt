...
abstract class UserDirectoryUserItem : VectorEpoxyModel<UserDirectoryUserItem.Holder>(R.layout.item_known_user) {
    ...
    override fun bind(holder: Holder) {
        ...
        // Show username (localpart of @username:server) as the primary label
        val username = matrixItem.id.removePrefix("@").substringBefore(":")
        val displayName = matrixItem.displayName?.takeIf { it.isNotBlank() }
        holder.nameView.text = username
        // Always hide display/profile name in search result as requested
