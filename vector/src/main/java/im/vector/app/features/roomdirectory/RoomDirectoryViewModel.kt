...
class RoomDirectoryViewModel @AssistedInject constructor(
    ...
    private fun load(filter: String, roomDirectoryData: RoomDirectoryData) {
        // Use showAllRooms flag or if we're searching for an alias, don't filter it out
        if (!showAllRooms && !explicitTermFilter.canSearchFor(filter) && !filter.startsWith("#")) {
            setState {
                copy(
        ...
        currentJob = viewModelScope.launch {
            ...
            val newPublicRooms = data.chunk.orEmpty()
                    .filter {
                        // Display all public rooms as per user request, bypassing explicit filter if needed
                        true // showAllRooms || explicitTermFilter.isValid("${it.name.orEmpty()} ${it.topic.orEmpty()}")
                    }
