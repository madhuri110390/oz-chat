...
//        window.navigationBarColor = ContextCompat.getColor(this, im.vector.lib.ui.styles.R.color.black_alpha)
    ...
    override fun onShare() {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val brandedFile = if (session != null && getMimeTypeFromUri(this@VectorAttachmentViewerActivity, file.toUri())?.startsWith("image/") == true) {
                try {
                    val originalBitmap = BitmapFactory.decodeFile(file.absolutePath)
                    if (originalBitmap != null) {
                        val username = session.myUserId.substringBefore(":").removePrefix("@")
                        val brandedBitmap = ImageBrandingUtils.brandImage(this@VectorAttachmentViewerActivity, originalBitmap, username)
                        ImageBrandingUtils.saveBrandedImage(this@VectorAttachmentViewerActivity, brandedBitmap) ?: file
                    } else {
                        file
                    }
                } catch (e: Exception) {
                    Timber.e(e, "Failed to brand image")
                    file
                }
            } else {
                file
...
//                val caption = if (session != null && getMimeTypeFromUri(this@VectorAttachmentViewerActivity, brandedFile.toUri())?.startsWith("image/") == true) {
//                    val username = session.myUserId.substringBefore(":").removePrefix("@")
//                    ImageBrandingUtils.getShareCaption(username)
//                } else {
//                    null
//                }
                ...
                withContext(Dispatchers.Main) {
                    shareMedia(
                            ...
                            brandedFile,
                            getMimeTypeFromUri(this@VectorAttachmentViewerActivity, brandedFile.toUri()),
                            null
                    )
                }
            }
