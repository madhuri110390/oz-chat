...
//                val profileUrl = "https://openzippers.com/profile/$username"
//                openUrlInChromeCustomTab(requireContext(),profileUrl)
//                true
//            }
//        }
        mUserAvatarPreference.setOnPreferenceClickListener {

            val username = session.myUserId
                    ?.removePrefix("@")
                    ?.substringBefore(":")
            ...
            context?.let { ctx ->
                openUrlInChromeCustomTab(ctx, null, profileUrl)
            }
        ...
        mDisplayNamePreference.let {
            it.onPreferenceClickListener = Preference.OnPreferenceClickListener {
                openUrlInChromeCustomTab(requireContext(), null, "https://openzippers.com/settings")
                true
            }
        }
...
//        }
        ...
        findPreference<VectorPreference>(VectorPreferences.SETTINGS_LOGGED_IN_PREFERENCE_KEY)?.let {
            val username = session.myUserId.substring(1).substringBefore(":")
            it.summary = username
        }
    ...
    private fun onPasswordUpdateClick() {
        activity?.let { activity ->
            ...
            dialog.setOnShowListener {
                val updateButton = dialog.getButton(BUTTON_POSITIVE)
                val cancelButton = dialog.getButton(BUTTON_NEGATIVE)
