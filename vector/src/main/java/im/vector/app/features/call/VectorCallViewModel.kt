...
class VectorCallViewModel @AssistedInject constructor(
    ...
    private val callListener = object : WebRtcCall.Listener {
        ...
        override fun onStateUpdate(call: MxCall) {
            ...
            val iceState = if (callState is CallState.Connected) callState.iceConnectionState else MxPeerConnectionState.DISCONNECTED
        ...
        override fun onVideoRequestReceived(mxCall: MxCall) {
            _viewEvents.post(VectorCallViewEvents.ShowVideoRequestDialog(mxCall.callId))
        }

        /*        override fun onVideoRequestAccepted(mxCall: MxCall) {
            _viewEvents.post(VectorCallViewEvents.VideoRequestAccepted(mxCall.callId))
        }*/

        //        override fun onRemoteScreenShareChanged(isSharing: Boolean) {
//            setState { copy(isRemoteScreenSharing = isSharing) }
//        }
        override fun onRemoteScreenShareChanged(isSharing: Boolean) {
            setState { copy(isRemoteScreenSharing = isSharing) }
            withState { currentState ->
                if (isSharing && currentState.isLocalScreenSharing) {
                    _viewEvents.post(VectorCallViewEvents.ShowScreenShareConflictDialog)
                }
        ...
        override fun onCallEnded(callId: String) {
            withState { state ->
                if (state.otherKnownCallInfo?.callId == callId) {
                    setState { copy(otherKnownCallInfo = null, isLocalScreenSharing = false, isRemoteScreenSharing = false) }
                }
            }
        ...
        override fun onCallUpdateTypeReceived(mxCall: MxCall, update: CallUpdateTypeContent) {
            ...
            when (update.updateCallType) {
                UpdateCallType.VOICE -> {
                    withState { state ->
                        if (state.pendingVideoRequestCallId != null && state.pendingVideoRequestCallId == mxCall.callId) {
                            // This is a rejection of our video request
                            setState {
                                copy(
                                        isVideoCall = false,
                                        pendingVideoRequestCallId = null
                                )
                            }
                            _viewEvents.post(VectorCallViewEvents.VideoRequestRejected(mxCall.callId))
                        } else {
                            // Normal voice call update — also clears screen share state if it was active
                            setState { copy(isVideoCall = false, isVideoEnabled = false, isRemoteScreenSharing = false) }
                        }
                    }
                }
                UpdateCallType.VIDEO -> {
                    setState { copy(isVideoCall = true, isVideoEnabled = true, isRemoteScreenSharing = false) }
                }
                UpdateCallType.VIDEO_ACCEPT -> {
                    // This is when someone accepts our video request
                    withState { state ->
                        if (state.pendingVideoRequestCallId != null && state.pendingVideoRequestCallId == mxCall.callId) {
                            setState {
                                copy(
                ...
                UpdateCallType.SCREEN_SHARE -> {
                    ...
                    setState { copy(isRemoteScreenSharing = true) }
                }
                else -> Unit
            }
        }
        ...
        if (webRtcCall == null) {
            ...
            setState {
                copy(
                        ...
                        isLocalScreenSharing = webRtcCall.isSharingScreen(),
    ...
    private fun shouldActivateProximitySensor(call: WebRtcCall): Boolean {
        return try {
            val onHold = call.isLocalOnHold || call.isRemoteOnHold
            val sharing = call.isSharingScreen() // now safe
            // Proximity only for pure voice (no screen sharing, not video) and when not remote-on-hold
            !call.mxCall.isVideoCall && !sharing && !call.isRemoteOnHold && !onHold
        } catch (_: IllegalStateException) {
            // Tracks might be disposed while switching route / resuming; keep sensor off
            false
        } catch (_: Throwable) {
            false
        }
    ...
    override fun handle(action: VectorCallViewActions) = withState { state ->
        when (action) {
            ...
            VectorCallViewActions.SwitchToVideo -> {
                viewModelScope.launch {
                    val callId = call?.mxCall?.callId ?: return@launch
                    // Start camera immediately to improve loading time
                    call?.switchToVideoAsInitiator()
                    call?.mxCall?.sendVideoRequest()

                    // Step 2: update local state → show loader / waiting UI
                    setState { copy(isVideoCall = true, pendingVideoRequestCallId = callId) }
                    _viewEvents.post(VectorCallViewEvents.VideoRequestSent(callId))
                }
            }

            VectorCallViewActions.SwitchToVoice -> {
                viewModelScope.launch{
                    call?.switchToVoice()
                    setState { copy(isVideoCall = false, pendingVideoRequestCallId = null) }
                }
            }
            VectorCallViewActions.ToggleHoldResume -> {
                val isRemoteOnHold = state.isRemoteOnHold
                // Optimistic UI update so the button state updates immediately
                setState { copy(isRemoteOnHold = !isRemoteOnHold) }
            ...
            is VectorCallViewActions.ToggleScreenSharing -> {
                viewModelScope.launch {
                    handleToggleScreenSharing(state.isLocalScreenSharing)
                }
            }

            is VectorCallViewActions.StartScreenSharing -> {
                Timber.d("VectorCallViewModel: dispatching StartScreenSharing with capturer")
                call?.startSharingScreen(action.videoCapturer)
                proximityManager.stop()
                setState {
                    copy(isLocalScreenSharing = true)
                }
            }
            VectorCallViewActions.AcceptVideoRequest -> {
                val callId = call?.mxCall?.callId ?: return@withState
                call?.acceptVideoRequest()
                setState {
                    copy(
                            pendingVideoRequestCallId = null,
                            videoRequestAccepted = true,
                            isVideoEnabled = true,
                            isVideoCall = true
                    )
                }
                _viewEvents.post(VectorCallViewEvents.VideoRequestAccepted(callId))
            }
            VectorCallViewActions.RejectVideoRequest -> {
                val callId = call?.mxCall?.callId ?: return@withState
                call?.rejectVideoRequest()
                setState {
                    copy(
                            pendingVideoRequestCallId = null,
                            videoRequestAccepted = false,
                            isVideoEnabled = false,
                            isVideoCall = false
                    )
                }
                _viewEvents.post(VectorCallViewEvents.VideoRequestRejected(callId))
            }

            VectorCallViewActions.OpenInviteFriends -> {
                _viewEvents.post(VectorCallViewEvents.ShowInviteFriends)
            }
            else -> {}
        }
    }
    ...
    private suspend fun handleToggleScreenSharing(isSharingScreen: Boolean) {
        Timber.d("VectorCallViewModel received ToggleScreenSharing. isSharingScreen: $isSharingScreen")
        if (isSharingScreen) {
            call?.stopSharingScreen()
            setState {
                copy(isLocalScreenSharing = false)
            }
            _viewEvents.post(
