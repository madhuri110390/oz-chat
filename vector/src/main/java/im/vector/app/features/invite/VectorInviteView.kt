...
//        views.inviteAcceptView.commonClicked = ClickListener { callback?.onAcceptInvite() }
//        views.inviteRejectView.commonClicked = ClickListener { callback?.onRejectInvite() }
    }

    fun render(sender: RoomMemberSummary, mode: Mode = Mode.LARGE, changeMembershipState: ChangeMembershipState) {
        val senderUsername = sender.userId.removePrefix("@").substringBefore(":")
        if (mode == Mode.LARGE) {
            ...
            views.inviteNameView.text = senderUsername
            ...
            views.inviteLabelView.text = context.getString(CommonStrings.invited_by, senderUsername)
        }
