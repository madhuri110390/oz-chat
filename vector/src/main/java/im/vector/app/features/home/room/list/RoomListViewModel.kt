...
class RoomListViewModel @AssistedInject constructor(
    ...
    private val directoryUsersLiveData = MutableLiveData<List<User>>()
    ...
    init {
        ...
        onEach(RoomListViewState::directoryUsers) { asyncUsers ->
            directoryUsersLiveData.postValue(asyncUsers.invoke() ?: emptyList())
        }

        // ✅ CORRECT - only search when filter is not blank
        onEach(RoomListViewState::roomFilter) { filter ->
            if (filter.isNotBlank()) {
                executeGlobalSearch(filter)
            } else {
                setState { copy(directoryUsers = Uninitialized) }
            }
        }
    }

    private fun executeGlobalSearch(filter: String) {
        viewModelScope.launch {
            setState { copy(directoryUsers = Loading()) }
            try {
                val openZipperResult = searchOpenZipperUsers(filter)
                val searchResult = if (filter.isBlank()) {
                    emptyList()
                } else {
                    try {
                        session.userService().searchUsersDirectory(filter, 100, emptySet())
                    } catch (failure: Throwable) {
                        Timber.w(failure, "Matrix directory search failed")
                        emptyList()
                    }
                }
                val combinedResult = (searchResult + openZipperResult)
                        .groupBy { it.userId }
                        .map { (_, users) ->
                            users.find { !it.displayName.isNullOrBlank() } ?: users.first()
                        }
                        .sortedBy { it.displayName?.lowercase() }
                setState { copy(directoryUsers = Success(combinedResult)) }
            } catch (e: Exception) {
                setState { copy(directoryUsers = Fail(e)) }
            }
        }
    }
    ...
    private suspend fun searchOpenZipperUsers(search: String): List<User> = withContext(Dispatchers.IO) {
        ...
        try {
            val userId = if (searchTerm.contains("@")) searchTerm else "@${searchTerm.lowercase()}:${session.myUserId.substringAfter(":")}"
            val profile = session.profileService().getProfileAsUser(userId)
            val displayName = profile.displayName.takeIf { !it.isNullOrBlank() } ?: searchTerm
    ...
    private val roomListSectionBuilder = RoomListSectionBuilder(
            ...
            directoryUsersLiveData
    )

    val sections: List<RoomsSection> by lazy {
        roomListSectionBuilder.buildSections(initialState.displayMode)
    }
    private fun handleSelectUserDirectRoom(action: RoomListAction.SelectUserDirectRoom) {
        viewModelScope.launch(Dispatchers.IO) {
            val existingRoomId = session.roomService()
                    .getExistingDirectRoomWithUser(action.user.userId)
            val roomSummary = existingRoomId?.let { session.getRoomSummary(it) }
            if (roomSummary != null) {
                _viewEvents.post(RoomListViewEvents.SelectRoom(roomSummary, false))
            } else {
                _viewEvents.post(RoomListViewEvents.NavigateToMxToBottomSheet(
                        session.permalinkService().createPermalink(action.user.userId) ?: return@launch
                ))
            }
        }
    }
    override fun handle(action: RoomListAction) {
        when (action) {
            is RoomListAction.SelectUserDirectRoom -> handleSelectUserDirectRoom(action)  // ADD THIS
