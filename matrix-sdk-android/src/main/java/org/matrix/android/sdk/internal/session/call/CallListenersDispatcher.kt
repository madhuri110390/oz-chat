...
internal class CallListenersDispatcher(private val listeners: Set<CallListener>) : CallListener {
    ...
    override fun onCallUpdateTypeReceived(mxCall: MxCall, update: CallUpdateTypeContent) = dispatch {
        it.onCallUpdateTypeReceived(mxCall, update)
    }

    override fun onVideoRequestReceived(mxCall: MxCall) = dispatch {
        it.onVideoRequestReceived(mxCall)
    }

    override fun onVideoRequestAccepted(mxCall: MxCall) =dispatch {
        it.onVideoRequestAccepted(mxCall)
    }
