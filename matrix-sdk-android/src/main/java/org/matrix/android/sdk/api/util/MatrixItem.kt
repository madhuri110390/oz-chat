...
////            is RoomItem -> roomDisplayName ?: displayName
////            is RoomAliasItem -> roomDisplayName ?: displayName
////            is UserItem -> id.substringAfter("@").substringBefore(":")// Always use Matrix ID so initial comes from username, not profile name
////            else -> displayName
////        }
//
...
fun firstLetterOfDisplayName(): String {
    ...
    // For UserItem we already have the single letter, return directly
    if (this is UserItem) return name.ifEmpty { "?" }

    // For all other items (rooms, spaces etc.) — always single letter
...
//        } ?: id
//
//        val parts = name.split(Regex("[\\s._-]+")).filter { it.isNotBlank() }
//
//        return when {
//            parts.isEmpty() -> "?"
//            parts.size == 1 -> parts[0].take(1)
//            else -> "${parts[0][0]}${parts[1][0]}"
//        }.uppercase()
//    }
    companion object {
