...
internal class CallSignalingHandler @Inject constructor(
    ...
    fun onCallEvent(event: Event) {
        when (event.getClearType()) {
            ...
            EventType.CALL_UPDATE_TYPE ->{
                val content = event.getClearContent().toModel<CallUpdateTypeContent>() ?: return
                handleCallUpdateType(content)
            }
    ...
    private fun handleCallUpdateType(update: CallUpdateTypeContent) {
        val activeCall = activeCallHandler.getCallWithId(update.callId ?: return) ?: return

        // Ignore our own echoed update
        if (activeCall.ourPartyId == update.partyId) {
            Timber.tag(loggerTag.value).v("Ignoring CALL_UPDATE_TYPE from our own partyId: ${update.partyId}")
            return
        }

        Timber.tag(loggerTag.value).v(
                "CALL_UPDATE_TYPE received: callId=${update.callId}, " +
                        "type=${update.updateCallType}, from partyId=${update.partyId}"
        )

        callListenersDispatcher.onCallUpdateTypeReceived(activeCall, update)
    }
