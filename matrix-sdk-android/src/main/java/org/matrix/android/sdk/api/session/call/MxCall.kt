...
interface MxCall : MxCallDetail {
    ...
    /**
     * Send call update type m.call.call_update_type to the other participant. For Oz-Chat Original functionality.
     */
    fun sendUpdateType(content: Content)

    /**
     * 🔹 New helpers for video upgrade flow.
     */
    fun sendVideoRequest()
    fun sendVideoAccept()
    fun sendVideoReject()

    /**
     * Signal to the remote peer that screen sharing has started.
     */
    fun sendScreenShareStart()

    /**
     * Signal to the remote peer that screen sharing has stopped (back to voice).
     */
