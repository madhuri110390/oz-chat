...
interface CallListener {
    ...
    /**
     * Called when we receive a m.call.update_type event from the other side.
     */
    fun onCallUpdateTypeReceived(mxCall: MxCall, update: CallUpdateTypeContent)

    /**
     * Called when the remote party requests to upgrade an ongoing voice call to video.
     * (maps to UpdateCallType.VIDEO_REQUEST)
     */
    fun onVideoRequestReceived(mxCall: MxCall)

    /**
     * Called when the local party receives confirmation that their video request
     * has been accepted by the remote side.
     * (maps to UpdateCallType.VIDEO_ACCEPT)
     */
    fun onVideoRequestAccepted(mxCall: MxCall)
}
