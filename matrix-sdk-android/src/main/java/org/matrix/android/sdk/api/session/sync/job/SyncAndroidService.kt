...
/**
 * Dummy implementation of SyncAndroidService that does absolutely nothing.
 * All sync, wake locks, and rescheduling logic have been removed to avoid
 * Play Console warnings (foreground service, background work, battery usage).
 */
abstract class SyncAndroidService : Service() {

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Timber.d("SyncAndroidService.onStartCommand() - disabled, stopping immediately")
        stopSelf()
        return START_NOT_STICKY
    }

    override fun onDestroy() {
        Timber.d("SyncAndroidService.onDestroy() - no cleanup needed")
        // Do nothing – no resources to release
    }

    override fun onBind(intent: Intent?): IBinder? = null

    // Abstract methods – children must implement, but they will be no‑ops
    abstract fun provideMatrix(): org.matrix.android.sdk.api.Matrix
    abstract fun getDefaultSyncTimeoutSeconds(): Int
    abstract fun getDefaultSyncDelaySeconds(): Int
    abstract fun onStart(isInitialSync: Boolean)
    abstract fun onRescheduleAsked(sessionId: String, syncTimeoutSeconds: Int, syncDelaySeconds: Int)
