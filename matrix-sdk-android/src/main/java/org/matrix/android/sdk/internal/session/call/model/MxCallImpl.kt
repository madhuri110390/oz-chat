...
import org.matrix.android.sdk.api.session.events.model.EventType.CALL_UPDATE_TYPE
...
internal class MxCallImpl(
    ...
    override fun sendUpdateType(content: Content) {
        Timber.tag(loggerTag.value).v("sendUpdateType $callId")
    ...
    override fun sendVideoRequest() {
        Timber.tag(loggerTag.value).v("sendVideoRequest $callId")
        val content = CallUpdateTypeContent(
                callId = callId,
                partyId = ourPartyId,
                version = MxCall.VOIP_PROTO_VERSION.toString(),
                updateCallType = UpdateCallType.VIDEO_REQUEST
        )
        sendUpdateType(content.toContent())
    }

    override fun sendVideoAccept() {
        Timber.tag(loggerTag.value).v("sendVideoAccept $callId")
        val content = CallUpdateTypeContent(
                callId = callId,
                partyId = ourPartyId,
                version = MxCall.VOIP_PROTO_VERSION.toString(),
                updateCallType = UpdateCallType.VIDEO_ACCEPT
        )
        sendUpdateType(content.toContent())
    }

    override fun sendVideoReject() {
        Timber.tag(loggerTag.value).v("sendVideoReject $callId")
        val content = CallUpdateTypeContent(
                callId = callId,
                partyId = ourPartyId,
                version = MxCall.VOIP_PROTO_VERSION.toString(),
                updateCallType = UpdateCallType.VOICE
        )
        sendUpdateType(content.toContent())
    }

    override fun sendScreenShareStart() {
        Timber.tag(loggerTag.value).v("sendScreenShareStart $callId")
        val content = CallUpdateTypeContent(
                callId = callId,
                partyId = ourPartyId,
                version = MxCall.VOIP_PROTO_VERSION.toString(),
                updateCallType = UpdateCallType.SCREEN_SHARE
        )
        sendUpdateType(content.toContent())
    }

    override fun sendScreenShareStop() {
        Timber.tag(loggerTag.value).v("sendScreenShareStop $callId")
        val content = CallUpdateTypeContent(
                callId = callId,
                partyId = ourPartyId,
                version = MxCall.VOIP_PROTO_VERSION.toString(),
